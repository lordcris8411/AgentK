...
for (const entry of await readdir(packages, { withFileTypes: true })) {
  ...
  if (manifest.apiVersion !== 1 || typeof manifest.id !== "string" || typeof manifest.worker !== "string")
    throw new Error(`Language Pack '${entry.name}' has an invalid manifest`);
  const source = join(directory, "worker.ts");
  const output = join(directory, manifest.worker);
  const stagingDirectory = join(directory, ".agent-k-language-pack-build");
  const stagingOutput = join(stagingDirectory, "worker.js");
  if (!existsSync(source)) throw new Error(`Language Pack '${manifest.id}' is missing worker.ts`);
  ...
  try {
    ...
    if (!checkOnly) {
      if (!existsSync(stagingOutput))
        throw new Error(`Language Pack '${manifest.id}' did not produce ${manifest.worker}`);
