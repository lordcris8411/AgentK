...
const COLOR_KEYS = ["surface-app", "surface-panel", "surface-raised", "surface-hover", "surface-active", "border-color", "border-strong", "text-primary", "text-secondary", "text-muted", "accent", "selection-background", "selection-foreground", "scrollbar-thumb", "scrollbar-thumb-hover", "danger", "info", "success", "warning", "modal-overlay", "icon-primary", "icon-secondary"];
...
const OPTIONAL_COMPONENT_KEYS = ["inactive-item-foreground"];
...
function normalize(input: unknown, builtin = false): ThemeDefinition {
  ...
  const fallbackComponents: Record<string, string> = {
    ...
    "inactive-item-foreground": colors["text-secondary"]!,
  ...
  const components: Record<string, string> = Object.keys(asObject(source.components)).length
    ? group(source.components, COMPONENT_KEYS, "components")
    : fallbackComponents;
  const optionalComponents = asObject(source.components);
  for (const key of OPTIONAL_COMPONENT_KEYS) {
    const value = asString(optionalComponents[key]);
    if (value && !validColor(value)) throw new Error(`Invalid components.${key}`);
    components[key] = value ?? fallbackComponents[key] ?? colors["text-secondary"]!;
  }
