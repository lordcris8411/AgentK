...
function imageScaleTransform(scale: number) {
...
function ImagePreview({
  ...
  useLayoutEffect(() => {
    // The active zoom loop owns the visual transform until it settles. A
    // React commit in the middle of that loop must not put an older scale back.
    if (zoomAnimationRef.current !== undefined) return;
    currentScaleRef.current = actualScale;
    targetScaleRef.current = actualScale;
    if (canvasRef.current)
      canvasRef.current.style.transform = imageScaleTransform(actualScale);
    if (zoomValueRef.current)
      zoomValueRef.current.textContent = `${Math.round(actualScale * 100)}%`;
  }, [actualScale]);

  const renderScale = (nextScale: number) => {
    currentScaleRef.current = nextScale;
    if (canvasRef.current)
      canvasRef.current.style.transform = imageScaleTransform(nextScale);
    if (zoomValueRef.current)
      zoomValueRef.current.textContent = `${Math.round(nextScale * 100)}%`;
  };
  ...
  const animateActualScaleBy = (amount: number) => {
    ...
    const animate = () => {
      ...
      // Keep the whole zoom on one compositor-owned transform. Updating React
      // state here forced a full preview render on every animation frame and
      // could present the canvas and surrounding UI from different frames.
      renderScale(settled);
      if (finished) {
        zoomAnimationRef.current = undefined;
        setScale(settled);
