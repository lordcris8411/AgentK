import type { LanguagePack } from "../../lib/desktop";
...
export function debugProviders(plugins: LanguagePack[]): DebugProvider[] {
...
export function rankDebugProviders(providers: DebugProvider[], contextFile?: string, preferred?: string): DebugProvider[] {
  ...
  return [...providers].sort((left, right) => {
    const identity = (provider: DebugProvider) => `${provider.packId}:${provider.id}`;
...
export function debugProviderIdentity(provider: Pick<DebugProvider, "id" | "packId">): string {
  return `${provider.packId}:${provider.id}`;
}
