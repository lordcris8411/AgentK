...
fn get_runtime_info() -> RuntimeInfo {
    let package = include_str!("../../../pi/packages/coding-agent/package.json");
    let pi_version = serde_json::from_str::<Value>(package)
        .ok()
        .and_then(|value| {
            value
                .get("version")
...
#[tauri::command]
fn list_browsers() -> Vec<BrowserOption> {
...
async fn pi_command(
    ...
    // `login_provider` can remain pending while the renderer displays a form.
    // Run all RPC waits off the IPC executor so the matching UI response can
    // enter through `pi_extension_ui_response` without freezing the WebView.
    ...
    let response = tauri::async_runtime::spawn_blocking(move || request_rpc(&bridge, command))
        .await
