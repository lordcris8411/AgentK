...
export function ConversationWorkspace({
  ...
  const updateScrollMetrics = (list = messageListRef.current) => {
    ...
    setScrollMetrics({
      visible: maxScroll > 1,
      height,
      top: maxScroll > 0 ? (list.scrollTop / maxScroll) * (100 - height) : 0,
    });
  };
  ...
  useEffect(() => {
    const list = messageListRef.current;
    if (!list) return;
    const observer = new ResizeObserver(() => updateScrollMetrics(list));
  ...
  const revertTurn = async (id: string, query: string, calls: ToolCall[]) => {
    ...
    try {
      ...
      const navigation = (await desktop.command({
        // Tree navigation changes which extension state entries are active.
        // Clear host-owned UI first so stale status/widgets cannot survive if
        // the target branch does not set replacements.
        type: "navigate_tree",
        entryId: target.entryId,
        summarize: false,
      }, session.runtimeId)) as { editorText?: string; cancelled?: boolean };
      ...
      const editorText = navigation.editorText ?? query;
      setDraft(editorText);
      requestAnimationFrame(() => placeCaretAtEnd(composerRef.current));
    } catch (cause) {
  ...
  const sentItems = visibleItems.filter(
    (item) =>
      item.role === "user" &&
      (item.content.trim() || item.images?.length || item.localImageUrls?.length),
  );
    ...
    >
      ...
      <div className="message-scroll-region">
        <section
          ...
          ref={messageListRef}
        >
          {timeline(visibleItems).map((entry, index, entries) => {
        ...
        <div
          ...
          className={
            scrollMetrics.visible
              ? "conversation-scrollbar is-visible"
      ...
      {sentItems.length > 0 && (
        <nav
          ...
          onMouseLeave={() => setPreviewMessageId(undefined)}
        >
          {sentItems.map((item, index) => {
            const distance = previewMessageId
              ? Math.abs(
                  index -
                    sentItems.findIndex(
                      (entry) => entry.id === previewMessageId,
                    ),
                )
              : -1;
            const level =
              distance === 0
                ...
                : distance === 1
                  ...
                  : distance === 2
                    ? " is-far"
                    : "";
            const start = items.findIndex((entry) => entry.id === item.id);
            const nextUser = items.findIndex(
              (entry, itemIndex) => itemIndex > start && entry.role === "user",
            );
            const turn = items.slice(
              start + 1,
              nextUser < 0 ? undefined : nextUser,
            );
            const answer = turn
              .filter(
