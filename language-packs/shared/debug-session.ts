...
export type DebugSnapshot = {
  adapter?: DebugAdapterLaunch["adapter"];
...
export type DebugAdapterLaunch = { adapter: "coreclr" | "lldb" | "pwa-node"; args: string[]; command: string; env?: NodeJS.ProcessEnv; transport?: { host: string; kind: "tcp"; port: number } };
...
export class DebugSession {
  ...
  private adapter?: DebugAdapterLaunch["adapter"];
  ...
  async start(input: DebugStartConfiguration, trustedProgramRoots: string[] = []): Promise<DebugSnapshot> {
    ...
    if (mode === "dump") {
      if (launch.adapter !== "lldb") throw new Error(`${launch.adapter} does not support dump debugging`);
    ...
    const child = spawn(launch.command, launch.args, {
      cwd,
      env: launch.env ?? { ...process.env, PATH: process.env.PATH?.split(delimiter).filter(Boolean).join(delimiter) },
      stdio: "pipe",
      windowsHide: true,
    });
    this.child = child;
    if (launch.transport?.kind === "tcp") {
      child.stdout.on("data", (data: Buffer) => this.appendOutput(data.toString("utf8")));
      this.adapterSocket = await this.connectTcpAdapter(launch.transport.host, launch.transport.port);
      this.adapterSocket.on("data", (data: Buffer) => this.consume(data));
      this.adapterSocket.on("error", (cause) => this.fail(cause));
    } else child.stdout.on("data", (data: Buffer) => this.consume(data));
    ...
    try {
      ...
      const nativeStartArguments = mode === "launch"
        ? {
          args: input.args ?? [],
          cwd,
      ...
      const startArguments = launch.adapter === "coreclr"
        ? mode === "launch" ? {
          args: input.args ?? [], cwd, ...(input.environment ? { env: input.environment } : {}),
          name: program?.split(/[\\/]/).pop() ?? "program", program, request: "launch", stopAtEntry: input.stopOnEntry === true, type: "coreclr",
        } : { name: `Process ${input.processId}`, processId: input.processId, request: "attach", type: "coreclr" }
        : launch.adapter === "pwa-node"
          ? mode === "launch" ? {
            args: input.args ?? [], cwd, ...(input.environment ? { env: input.environment } : {}),
            name: program?.split(/[\\/]/).pop() ?? "program", program, request: "launch", ...(input.runtimeExecutable ? { runtimeExecutable: input.runtimeExecutable } : {}), stopOnEntry: input.stopOnEntry === true, type: "pwa-node",
          } : { name: `Process ${input.processId}`, processId: input.processId, request: "attach", type: "pwa-node" }
          : nativeStartArguments;
  ...
  private async disconnect(terminateDebuggee: boolean): Promise<DebugSnapshot> {
    ...
    if (this.child === child) {
      this.child = undefined;
      this.adapterSocket?.destroy(); this.adapterSocket = undefined;
      await stopChildProcess(child, "debug adapter");
    }
  ...
  private request(command: string, args: unknown, timeoutMs = 10_000): Promise<unknown> {
    ...
    return new Promise((resolveRequest, reject) => {
      ...
      const input = this.adapterSocket ?? child.stdin;
      input.write(`Content-Length: ${Buffer.byteLength(json)}\r\n\r\n${json}`, (cause) => {
        if (!cause) return;
  ...
  private async connectTcpAdapter(host: string, port: number): Promise<Socket> {
    const deadline = Date.now() + 10_000;
    for (;;) {
      try {
        return await new Promise<Socket>((resolveSocket, rejectSocket) => {
          const socket = connect({ host, port });
          socket.once("connect", () => { socket.removeAllListeners("error"); resolveSocket(socket); });
          socket.once("error", rejectSocket);
        });
      } catch (cause) {
        if (Date.now() >= deadline || !this.child || this.child.exitCode !== null) throw cause;
  ...
  private async syncFunctionBreakpoints(): Promise<void> {
    if (this.capabilities.supportsFunctionBreakpoints !== true) return;
