...
export class CppService {
  ...
  private readonly downloadConfirmations = new Map<string, { resolve(confirmed: boolean): void; timeout: ReturnType<typeof setTimeout> }>();
  private nextDownloadConfirmation = 1;
  ...
  private readonly systemTools: Record<string, { command?: string }>;
  constructor(cachePath: string, emit: (event: Record<string, unknown>) => void, systemTools: Record<string, { command?: string }> = {}) {
  ...
  private publish(entry: Entry) { this.emit({ type: "language_pack_project", project: this.public(entry) }); }
  ...
  async load(rootInput: string): Promise<CppProject> {
    ...
    try {
      ...
      await this.start(entry, this.toolPath("clangd", bin), commandsDir, bin, buildEnvironment);
      return this.public(entry);
    } catch (cause) { await rm(`${join(this.cachePath, "cpp-toolchain", process.platform, process.arch)}.partial`, { recursive: true, force: true }); if (this.cancellationRequested) { this.projects.delete(root); this.emit({ type: "language_pack_project_removed", root }); return { ...this.public(entry), status: "stopped" }; } entry.status = "failed"; entry.error = cause instanceof Error ? cause.message : String(cause); this.publish(entry); this.emit({ type: "language_pack_progress", stage: "failed", error: entry.error, detail: entry.error }); return this.public(entry); }
  }
  async unload(root: string) { const entry = this.projects.get(await realpath(root)); if (!entry) return; const timer = this.reconfigureTimers.get(entry.root); if (timer) clearTimeout(timer); this.reconfigureTimers.delete(entry.root); this.clearDebugTargetDiscovery(entry.root); await this.stop(entry); this.projects.delete(entry.root); this.emit({ type: "language_pack_project_removed", root: entry.root }); }
  async restart(root: string) { await this.unload(root); return this.load(root); }
  async shutdown() { this.rejectDownloadConfirmations(); this.debugProvisionAbort?.abort(); this.debug.shutdown(); this.debugTargetDiscovery.clear(); this.cmakeQueues.clear(); for (const timer of this.reconfigureTimers.values()) clearTimeout(timer); this.reconfigureTimers.clear(); await Promise.all([...this.projects.values()].map((entry) => this.stop(entry))); this.projects.clear(); }
  cancel(): void { this.cancellationRequested = true; this.rejectDownloadConfirmations(); this.provisionAbort?.abort(); this.debugProvisionAbort?.abort(); this.activeCommand?.kill(); }
  respondConfirmation(id: string, confirmed: boolean): boolean {
    const pending = this.downloadConfirmations.get(id);
    if (!pending) return false;
    clearTimeout(pending.timeout);
    this.downloadConfirmations.delete(id);
  ...
  async agent(input: Record<string, unknown>): Promise<unknown> {
    ...
    if (action === "debug.stop") return this.debugStop(typeof input.sessionId === "string" ? input.sessionId : undefined);
    if (!workspace) throw new Error("workspace is required");
    if (action === "project.load") return this.load(workspace);
    if (action === "project.status") return this.projects.has(workspace) ? this.public(this.projects.get(workspace)!) : { root: workspace, name: basename(workspace), status: "stopped" };
    if (action === "project.restart") return this.restart(workspace);
    if (action === "project.unload") return this.unload(workspace);
    if (action === "debug.configurations") return this.debugConfigurations(workspace, typeof input.file === "string" ? resolve(workspace, input.file) : undefined, input.refresh === true, input.configuration);
    if (action === "debug.start" || action === "debug.attach") return this.debugStart({ root: workspace, mode: action === "debug.attach" ? "attach" : "launch", ...(typeof input.processId === "number" ? { processId: input.processId } : {}), ...(typeof input.program === "string" ? { program: resolve(workspace, input.program) } : {}), args: Array.isArray(input.args) ? input.args.filter((value): value is string => typeof value === "string") : [] });
    if (action === "build" || action === "test" || action === "run") return this.executeProjectAction(workspace, action, input);
    if (!action.startsWith("language.")) throw new Error(`Unsupported C/C++ Language Pack action: ${action}`);
    const entry = this.projects.get(workspace); if (!entry || (entry.status !== "ready" && entry.status !== "indexing")) throw new Error("C/C++ project is not ready");
    ...
    if (file && !isInside(workspace, file)) throw new Error("Language action file escapes the workspace");
    ...
    const target = methods[action]; if (!target) throw new Error(`Unsupported C/C++ semantic action: ${action}`);
    return this.lsp(file ?? join(workspace, "__workspace__.cpp"), target[0], target[1]);
  }
  ...
  private async executeProjectAction(workspace: string, action: "build" | "run" | "test", input: Record<string, unknown>): Promise<Record<string, unknown>> {
    const configuration = this.cmakeBuildConfiguration(input.configuration); const prepared = await this.prepareDebugTargets(workspace, configuration);
    const cmake = this.toolPath("cmake", prepared.bin);
    let command = cmake; let args: string[];
    if (action === "build") args = ["--build", prepared.build, "--config", configuration];
    else if (action === "test") { command = join(dirname(cmake), process.platform === "win32" ? "ctest.exe" : "ctest"); args = ["--test-dir", prepared.build, "-C", configuration, "--output-on-failure"]; }
    else { const program = typeof input.program === "string" ? resolve(workspace, input.program) : undefined; if (!program || !isInside(workspace, program) && !isInside(prepared.build, program)) throw new Error("run requires a program inside the workspace or private build directory"); command = program; args = Array.isArray(input.args) ? input.args.filter((value): value is string => typeof value === "string") : []; }
    const started = Date.now(); return new Promise((resolveAction) => {
      const child = spawn(command, args, { cwd: prepared.build, env: prepared.environment, windowsHide: true, stdio: ["ignore", "pipe", "pipe"] }); this.activeCommand = child; let stdout = ""; let stderr = "";
      child.stdout.on("data", (value: Buffer) => { stdout += value.toString("utf8"); }); child.stderr.on("data", (value: Buffer) => { stderr += value.toString("utf8"); });
      child.once("error", (cause) => resolveAction({ code: -1, stdout, stderr: cause.message, command, args, cwd: prepared.build, artifacts: [prepared.build], durationMs: Date.now() - started, cancelled: this.cancellationRequested }));
      child.once("close", (code) => { if (this.activeCommand === child) this.activeCommand = undefined; resolveAction({ code: code ?? -1, stdout, stderr, command, args, cwd: prepared.build, artifacts: [prepared.build], durationMs: Date.now() - started, cancelled: this.cancellationRequested }); });
    });
  }
  ...
  private async stop(entry: Entry) {
    ...
    await this.enqueueNotification(entry, "exit", null).catch(() => false);
    entry.status = "stopped";
    for (const pending of entry.pending.values()) { clearTimeout(pending.timeout); pending.reject(new Error("clangd stopped")); }
    entry.pending.clear();
    await stopChildProcess(child, "clangd");
    if (entry.child === child) entry.child = undefined;
  }
  private async managedBin(): Promise<string> {
    const compilerId = process.platform === "win32" ? "compiler-windows" : "compiler-linux";
    if (["cmake", "ninja", "clangd", compilerId].every((id) => typeof this.systemTools[id]?.command === "string")) {
      const bin = dirname(this.systemTools.clangd!.command!);
      this.emit({ type: "language_pack_progress", stage: "preparing", detail: "Using compatible system C/C++ tools through an isolated environment" });
      return bin;
    }
    ...
    const compilers = marker && platform ? await this.managedCompilers(root).catch(() => undefined) : undefined;
    if (platform && marker === managedToolchainMarker(platform) && compilers && candidates.every((name) => existsSync(join(bin, `${name}${executable}`)))) { this.emit({ type: "language_pack_progress", stage: "preparing", detail: "Reusing cached C++ language tools" }); return bin; }
  ...
  private async provisionDebugger(root: string, archive: ReturnType<typeof managedDebuggerArchive> & {}, marker: string, signal: AbortSignal): Promise<DebugAdapterLaunch> {
    ...
    this.emit({ type: "language_pack_progress", stage: "preparing", tool: "LLDB debugger", detail: "Preparing managed LLDB debugger" });
    ...
    this.emit({ type: "language_pack_progress", stage: "ready", tool: "LLDB debugger", detail: "Managed LLDB debugger is ready" });
  ...
  private prepareDebugTargets(root: string, configuration: CMakeBuildConfiguration): Promise<{ bin: string; build: string; environment: NodeJS.ProcessEnv; targets: CMakeDebugTarget[] }> {
    ...
    const pending = (async () => {
      ...
      const compilers = await this.managedCompilers(dirname(bin));
      const targets = await this.withCMake(root, async () => {
        ...
        const cmake = this.toolPath("cmake", bin);
        ...
        await this.run(cmake, [
          ...
          `-DCMAKE_CXX_COMPILER=${compilers.cpp}`,
  ...
  private async provision(bin: string, signal: AbortSignal): Promise<string> {
    ...
    const missing: Array<{ archive: ToolchainArchive; tool: string }> = [];
    for (const [tool, archive] of Object.entries(archives))
      if (!await this.archiveMatches(join(archiveCache, archive.asset), archive.sha256)) missing.push({ archive, tool });
    if (missing.length && !await this.confirmToolchainDownload(missing)) {
      this.cancellationRequested = true;
      throw new Error("C++ toolchain download was not approved");
    }
    signal.throwIfAborted();
    await rm(staging, { recursive: true, force: true }); await mkdir(staging, { recursive: true });
    this.emit({ type: "language_pack_progress", stage: "preparing", detail: `Preparing managed ${platform} language tools` });
    for (const [tool, archive] of Object.entries(archives))
    ...
    await this.managedCompilers(staging);
    ...
    await rm(root, { recursive: true, force: true }); await rename(staging, root); this.emit({ type: "language_pack_progress", stage: "ready", detail: "Managed C++ language tools are ready" }); return bin;
  }
  private async confirmToolchainDownload(missing: Array<{ archive: ToolchainArchive; tool: string }>): Promise<boolean> {
    const requestId = `toolchain-${this.nextDownloadConfirmation++}`;
    const prompt = managedToolchainDownloadPrompt(missing);
    return new Promise<boolean>((resolveConfirmation) => {
      const timeout = setTimeout(() => {
        this.downloadConfirmations.delete(requestId);
        resolveConfirmation(false);
      }, 120_000);
      this.downloadConfirmations.set(requestId, { resolve: resolveConfirmation, timeout });
      this.emit({ type: "language_pack_confirmation_request", requestId, ...prompt });
    });
  }
  private rejectDownloadConfirmations(): void {
    for (const pending of this.downloadConfirmations.values()) {
      clearTimeout(pending.timeout);
      pending.resolve(false);
    }
    this.downloadConfirmations.clear();
  }
  private async managedCompilers(root: string): Promise<{ c: string; cpp: string }> {
    const system = this.systemTools[process.platform === "win32" ? "compiler-windows" : "compiler-linux"]?.command;
    if (system) return { c: /g\+\+(?:\.exe)?$/iu.test(system) ? system.replace(/g\+\+(\.exe)?$/iu, "gcc$1") : system.replace(/clang\+\+(\.exe)?$/iu, "clang$1"), cpp: system };
    const executable = process.platform === "win32" ? ".exe" : "";
    const c = await this.findExecutable(root, `clang${executable}`);
    const cpp = await this.findExecutable(root, `clang++${executable}`);
    if (!c || !cpp) throw new Error("Provisioned LLVM C/C++ compilers are missing");
    return { c, cpp };
  }
  private toolPath(id: "clangd" | "cmake" | "ninja", bin: string): string {
    return this.systemTools[id]?.command ?? join(bin, `${id}${process.platform === "win32" ? ".exe" : ""}`);
  }
  ...
  private environmentPath(environment: NodeJS.ProcessEnv): string {
    return environment.PATH ?? [...Object.entries(environment)].reverse().find(([key]) => key.toLocaleUpperCase("en-US") === "PATH")?.[1] ?? "";
  }
  private async cmakeEnvironment(bin: string): Promise<NodeJS.ProcessEnv> {
    const compilers = await this.managedCompilers(dirname(bin));
    const compilerBin = dirname(compilers.c);
    const inherited = Object.fromEntries(Object.entries(process.env).filter(([key]) => key.toLocaleUpperCase("en-US") !== "PATH"));
    this.emit({ type: "language_pack_progress", stage: "configuring", detail: "Using managed LLVM C/C++ compilers\n" });
    return {
      ...inherited,
      CC: compilers.c,
      CXX: compilers.cpp,
      PATH: [...new Set([bin, compilerBin, ...Object.values(this.systemTools).flatMap((tool) => tool.command ? [dirname(tool.command)] : []), ...(process.platform === "win32" ? [join(process.env.SystemRoot ?? "C:\\Windows", "System32")] : ["/usr/bin", "/bin"])])].join(delimiter),
    };
  }
  ...
  private async downloadRelease(owner: string, repo: string, tag: string, assetName: string, directory: string, archiveCache: string, tool: string, expectedSha256: string, signal: AbortSignal): Promise<void> {
    ...
    if (await this.archiveMatches(cachedArchive, expectedSha256)) { const size = (await stat(cachedArchive)).size; this.emit({ type: "language_pack_progress", stage: "downloading", tool, bytes: size, total: size, rate: 0, detail: `${assetName} (reused cache)` }); await copyFile(cachedArchive, archive); this.emit({ type: "language_pack_progress", stage: "extracting", tool, detail: assetName }); await this.extract(archive, directory, tool); this.emit({ type: "language_pack_progress", stage: "preparing", tool, detail: `Finalizing ${tool}` }); await rm(archive, { force: true }); return; }
    await rm(cachedArchive, { force: true }); const api = await fetchWithRetry(`https://api.github.com/repos/${owner}/${repo}/releases/tags/${tag}`, { headers: { Accept: "application/vnd.github+json", "User-Agent": "Agent-K" }, signal }); if (!api.ok) throw new Error(`Unable to locate ${tool} release (${api.status})`);
    const release = await api.json() as { assets?: Array<{ name: string; browser_download_url: string }> }; const asset = release.assets?.find((item) => item.name === assetName); if (!asset) throw new Error(`Release asset is unavailable for ${tool}`);
    await withNetworkRetry(async () => {
      await rm(archive, { force: true });
      const response = await fetchWithRetry(asset.browser_download_url, { headers: { "User-Agent": "Agent-K" }, signal }); if (!response.ok || !response.body) throw new Error(`Unable to download ${tool} (${response.status})`);
    ...
    this.emit({ type: "language_pack_progress", stage: "extracting", tool, detail: assetName }); await this.extract(archive, directory, tool); this.emit({ type: "language_pack_progress", stage: "preparing", tool, detail: `Finalizing ${tool}` }); await rm(archive, { force: true });
  }
  private async extract(archive: string, directory: string, tool: string): Promise<void> {
    const started = Date.now();
    const heartbeat = setInterval(() => this.emit({ type: "language_pack_progress", stage: "extracting", tool, detail: `${basename(archive)} · ${Math.max(1, Math.round((Date.now() - started) / 1_000))}s` }), 2_000);
    try {
      ...
      if (process.platform === "win32") {
        ...
        await this.run(join(systemRoot, "System32", "tar.exe"), ["-xf", archive, "-C", directory], directory, "");
      } else {
        const decompressor = /\.(?:tar\.xz|txz)$/iu.test(archive) ? "/usr/bin/xz" : "/usr/bin/gzip";
        if (!existsSync("/usr/bin/tar") || !existsSync(decompressor)) throw new Error("Declared Linux archive tools are unavailable");
        await this.run("/usr/bin/tar", ["-I", decompressor, "-xf", archive, "-C", directory], directory, "", { PATH: "/usr/bin:/bin" });
      }
  ...
  private async configureUnlocked(root: string, bin: string): Promise<{ commandsDir: string; environment: NodeJS.ProcessEnv }> {
    ...
    if (sourceCommands) {
      ...
      await mkdir(build, { recursive: true }); const cmake = this.toolPath("cmake", bin);
      if (!existsSync(cmake)) throw new Error("Managed CMake is unavailable");
      await this.run(cmake, ["-S", root, "-B", build, "-G", "Ninja", `-DCMAKE_C_COMPILER=${compilers.c}`, `-DCMAKE_CXX_COMPILER=${compilers.cpp}`, "-DCMAKE_EXPORT_COMPILE_COMMANDS=ON"], root, bin, environment);
  ...
  private async start(entry: Entry, clangd: string, commandsDir: string, bin: string, environment: NodeJS.ProcessEnv = process.env) {
        ...
        try {
          ...
          if (message.method === "textDocument/publishDiagnostics" && typeof message.params?.uri === "string" && Array.isArray(message.params.diagnostics)) {
            try {
              ...
              this.emit({ type: "language_pack_diagnostics", file, diagnostics: message.params.diagnostics });
    ...
    this.emit({ type: "language_pack_progress", stage: "ready", detail: bootstrapUri ? "C++ language service is ready; project indexing continues in the background" : "C++ language service is ready" });
  }
...
// This module is the built-in C/C++ Language Pack worker when launched by
// LanguagePackHost. Keeping the protocol here makes the pack lifecycle
...
if (typeof process.send === "function") {
  ...
  process.on("message", (message: WorkerRequest) => {
    ...
    if (message.method === "shutdown") {
      void (async () => {
        try { if (service) await service.shutdown(); reply({ id: message.id, result: undefined, type: "response" }); }
        catch (cause) { reply({ error: cause instanceof Error ? cause.message : String(cause), id: message.id, type: "response" }); }
      })();
      return;
    }
    void (async () => {
      try {
        if (message.method === "initialize") {
          ...
          const options = message.args?.[1] as { systemTools?: Record<string, { command?: string }> } | undefined;
          service = new CppService(cachePath, (event) => process.send?.({ event, type: "event" }), options?.systemTools);
