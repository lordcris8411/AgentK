...
test("pairs the clangd index with the private CMake build cache key", () => {
  assert.equal(
    privateClangdIndexDirectory(join("cache", "language-packs", "agent-k.cpp"), join("cache", "language-packs", "agent-k.cpp", "cpp-build", "workspace-hash")),
    join("cache", "language-packs", "agent-k.cpp", "cpp-index", "workspace-hash"),
  );
});
