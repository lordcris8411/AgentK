...
class RoutedExperts(PluggableLayer):
    ...
    def _process_mxfp4(self):
        ...
        self._maybe_initialize_nvcomp_fp8()

    def _maybe_initialize_nvcomp_fp8(self):
        import ctypes
        import os
        import re
        from pathlib import Path

        compressed_root = os.getenv("VLLM_DSV4_NVCOMP_FP8_DIR")
        if not compressed_root or not self.is_gpu_prefill_layer:
            return
        match = re.search(r"layers\.(\d+)\.", self.layer_name)
        if match is None:
            return
        layer = int(match.group(1))
        allowlist = os.getenv("VLLM_DSV4_NVCOMP_LAYERS")
        if allowlist:
            enabled_layers = {
                int(item.strip()) for item in allowlist.split(",")
                if item.strip()
            }
            if layer not in enabled_layers:
                return
        layer_dir = Path(compressed_root) / f"layer{layer}"
        group_size = 16
        num_groups = 256 // group_size
        paths = [layer_dir / f"group{group}.ans"
                 for group in range(num_groups)]
        if not all(path.is_file() for path in paths):
            return

        compressed = [
            torch.from_file(str(path), shared=False, size=path.stat().st_size,
                            dtype=torch.uint8)
            for path in paths
        ]
        if not hasattr(RoutedExperts, "nvcomp_fp8_state"):
            library_path = os.getenv(
                "VLLM_DSV4_NVCOMP_BRIDGE",
                "/opt/lvllmds4-x/sm89-tuning/nvcomp-test/"
                "libnvcomp_torch_bridge.so")
            library = ctypes.CDLL(library_path)
            library.nvcomp_bridge_create.argtypes = [
                ctypes.c_void_p, ctypes.c_size_t, ctypes.c_uint64]
            library.nvcomp_bridge_create.restype = ctypes.c_void_p
            library.nvcomp_bridge_create_capacity.argtypes = [
                ctypes.c_void_p, ctypes.c_size_t, ctypes.c_size_t,
                ctypes.c_uint64]
            library.nvcomp_bridge_create_capacity.restype = ctypes.c_void_p
            library.nvcomp_bridge_output_size.argtypes = [ctypes.c_void_p]
            library.nvcomp_bridge_output_size.restype = ctypes.c_size_t
            library.nvcomp_bridge_decode.argtypes = [
                ctypes.c_void_p, ctypes.c_void_p, ctypes.c_size_t,
                ctypes.c_void_p]
            library.nvcomp_bridge_decode.restype = ctypes.c_int
            library.nvcomp_bridge_error.restype = ctypes.c_char_p

            largest = max(range(num_groups),
                          key=lambda index: compressed[index].numel())
            transfer_stream = torch.cuda.Stream()
            capacity = max(
                path.stat().st_size
                for path in Path(compressed_root).glob("layer*/group*.ans")
            )
            context = library.nvcomp_bridge_create_capacity(
                compressed[largest].data_ptr(), compressed[largest].numel(),
                capacity, transfer_stream.cuda_stream)
            if not context:
                raise RuntimeError(library.nvcomp_bridge_error().decode())
            arena_bytes = 402751488
            if library.nvcomp_bridge_output_size(context) != arena_bytes:
                raise RuntimeError("unexpected nvCOMP decompressed arena size")
            arenas = [
                torch.empty(arena_bytes, device="cuda", dtype=torch.uint8)
                for _ in range(2)
            ]
            ...
            def make_views(storage):
                ...
                views = {}
                specs = (
                    ("w13_weight", (group_size, 4096, 4096),
                     torch.float8_e4m3fn),
                    ("w2_weight", (group_size, 4096, 2048),
                     torch.float8_e4m3fn),
                    ("w13_weight_scale", (group_size, 32, 32), torch.float32),
                    ("w2_weight_scale", (group_size, 32, 16), torch.float32),
                )
                for name, shape, dtype in specs:
                    ...
                    views[name] = storage[cursor:cursor + size].view(dtype).view(shape)
                    cursor += size
                return views

            stages = [make_views(arena) for arena in arenas]
            from vllm.model_executor.layers.fused_moe.config import (
            ...
            for begin in range(0, 256, group_size):
                expert_map = torch.full(
                    (256,), -1, device="cuda", dtype=torch.int32)
                expert_map[begin:begin + group_size] = torch.arange(
                    group_size, device="cuda", dtype=torch.int32)
                maps.append(expert_map)
            RoutedExperts.nvcomp_fp8_state = {
                "library": library,
                "context": context,
        ...
        state = RoutedExperts.nvcomp_fp8_state
        if max(tensor.numel() for tensor in compressed) > state["capacity"]:
            raise RuntimeError("nvCOMP compressed group exceeds shared capacity")
        self._nvcomp_fp8_compressed = compressed
        self._nvcomp_fp8_state = state
        logger.info(
            "Enabled nvCOMP ANS FP8 prefill for %s: compressed %.3f GiB, "
            "shared persistent GPU %.3f GiB",
            self.layer_name, sum(t.numel() for t in compressed) / 2**30,
            (sum(t.numel() for t in state["arenas"]) + state["capacity"]) /
            2**30,
        )
    ...
    def _gpu_prefill(self, hidden_states, topk_weights, topk_ids):
        import os
        nvcomp_min_tokens = int(os.getenv(
            "VLLM_DSV4_NVCOMP_MIN_TOKENS", "12288"))
        if (hasattr(self, "_nvcomp_fp8_compressed") and
                hidden_states.size(0) >= nvcomp_min_tokens):
            from vllm.model_executor.layers.fused_moe.fused_moe import fused_experts

            state = self._nvcomp_fp8_state
            library = state["library"]
            context = state["context"]
            transfer_stream = state["transfer_stream"]
            arenas = state["arenas"]
            stages = state["stages"]
            quant_configs = state["quant_configs"]
            maps = state["maps"]
            ready = state["ready"]
            consumed = state["consumed"]
            compressed = self._nvcomp_fp8_compressed
            compute_stream = torch.cuda.current_stream()
            timing = os.getenv("VLLM_DSV4_NVCOMP_MOE_TIMING") == "1"
            if timing:
            ...
            with torch.cuda.stream(transfer_stream):
                rc = library.nvcomp_bridge_decode(
                    context, compressed[0].data_ptr(), compressed[0].numel(),
                    arenas[0].data_ptr())
                if rc:
                    raise RuntimeError(library.nvcomp_bridge_error().decode())
                ready[0].record(transfer_stream)
            num_groups = len(compressed)
            for group in range(num_groups):
                slot = group & 1
                ...
                if group + 1 < num_groups:
                    ...
                    with torch.cuda.stream(transfer_stream):
                        ...
                        rc = library.nvcomp_bridge_decode(
                            context, compressed[group + 1].data_ptr(),
                            compressed[group + 1].numel(),
                            arenas[next_slot].data_ptr())
                        if rc:
                            raise RuntimeError(
                                library.nvcomp_bridge_error().decode())
                        ready[next_slot].record(transfer_stream)
                stage = stages[slot]
                partial = fused_experts(
                    hidden_states, stage["w13_weight"], stage["w2_weight"],
                    topk_weights, topk_ids, activation=self.activation,
                    apply_router_weight_on_input=self.apply_router_weight_on_input,
                    global_num_experts=self.local_num_experts,
                    expert_map=maps[group], quant_config=quant_configs[slot])
                output.add_(partial)
            ...
            if timing:
                ...
                logger.info(
                    "NVCOMP_MOE_TIMING layer=%s tokens=%d ms=%.3f",
                    self.layer_name, hidden_states.size(0),
